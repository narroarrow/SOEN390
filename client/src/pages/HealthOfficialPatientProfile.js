...
function HealthOfficialPatientProfile() {
  ...
  function getPatients() {
    ...
    Axios.get('http://localhost:8080/DoctorPatientProfile')
      .then((response) => {
  ...
  const getViewed = () => {
    //this function is called when the doctor patient profile page is loaded. It sets the useState patientList to the query result for patient info
    Axios.get('http://localhost:8080/Viewed')
      .then((response) => {
            ...
            {filteredPatients.map((val, key) => {
              ...
              return (
                <Grid item xs={12} sm={6} md={4} key={key}>
                  ...
                  <Link
                    to='/HealthOfficialViewingPatient'
                    state={{ ID: val.ID }}
                    style={{ textDecoration: 'none' }}
                  >
                    <Button
                      variant='outlined'
                      href='/HealthOfficialViewingPatient'
                      fullWidth
                    >
                      <CardHeader
                        fullwidth
