...
function DoctorViewingPatient() {
  ...
  if (viewedList.map((el) => el.ID).includes(location.state.ID)) {
  ...
  let markAsReviewed = () => {
    //When clicking the MARK AS REVIEWED button, this will send the patient and doctor information to the DB viewed table
    ...
    Axios.post('http://localhost:8080/markViewed', {
      ...
      PatientDocID: patientData.map((val, key) => {
        return val.DoctorID;
      })[0],
  ...
  let acceptChat = () => {
    //When clicking the ACCEPT CHAt button, this will update the ChatPermission attribute in the patient tale to true
  ...
  let isChatAcceptedArray = patientData.map((val, key) => {
    return val.ChatPermission;
  });
  ...
  let isNewPatientArray = patientData.map((val, key) => {
    return val.NewPatient;
  });
  ...
  let isChatRequestedArray = patientData.map((val, key) => {
    return val.ChatRequested;
  });
  if (isChatRequestedArray[0] === 1) {
    isChatRequested = true;
  }

  let patientsDoctorID = patientData.map((val, key) => {
    return val.DoctorID;
  })[0]; //retrieving the id of the patient's doctor
  ...
  return (
    <>
      {localStorage.getItem('role') !== ('Doctor' || 'Admin') && (
                ...
                <Grid container fullwidth spacing={1}>
                  ...
                  >
                    <Button
                      ...
                      sx={{ margin: 1 }}
                      variant='contained'
                      onClick={previousSymptoms}
                      href='/PreviousSymptoms'
                    >
