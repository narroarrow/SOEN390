...
function DoctorDashboard() {
  ...
  const handleApptMask = (index) => (event) => {
    ...
    let today = new Date();
    let dateOfAppt = new Date(
      notificationsList[index].aptDate.concat(
        ' ' + notificationsList[index].endTime
      )
    );
    ...
    if (Number(notificationsList[index].endTime.substr(0, 2)) < 8) {
      ...
      dateOfAppt = new Date(
        notificationsList[index].aptDate.concat(
          ' ' + apptTime + notificationsList[index].endTime.substr(2, 6)
        )
      );
    }
    ...
    if (today.getTime() > dateOfAppt.getTime()) {
      ...
      alert(
        'You can only remove appointment notifications after the appointment date!'
      );
    }
  };
  ...
  const data = [
    {
      argument: 'Healthy',
      value: totalStatusCounts.map((val, key) => {
  ...
  const myPatientsData = [
    {
      argument: 'Healthy',
      value: totalMyPatientsStatusCounts.map((val, key) => {
        return val.healthyCount;
      }),
    },
    {
  ...
  const handleFormMask = (index) => (event) => {
    ...
    event.preventDefault();
    let hours =
      Number(formNotificationsList[index].InfoTimestamp.substr(11, 2)) - 4;
    ...
    Axios.put('http://localhost:8080/maskFormNotification', {
      ...
      InfoTimestamp: formNotificationsList[index].InfoTimestamp.substr(0, 10)
        .concat(' ')
        .concat(hours.toString())
        .concat(formNotificationsList[index].InfoTimestamp.substr(13, 6)),
  ...
  function getDoctorPatients() {
    //returns all patient information for a given doctor using GET
    Axios.get('http://localhost:8080/doctorViewingTheirPatientData', {
      params: { id: tempDoctorID },
    })
      .then((response) => {
  ...
  function getPatientsPerDoctor() {
    //returns all patient information organized by doctor using GET
    Axios.get('http://localhost:8080/doctorViewingDoctorPatients')
      .then((response) => {
  ...
  function getAllPatients() {
    //returns all patient information using GET
    Axios.get('http://localhost:8080/doctorViewingAllPatientData')
      .then((response) => {
  ...
  function getStatusCountAllPatients() {
    // This will return the number of patients classified under each status for ALL patients
    Axios.get('http://localhost:8080/statusCountAllPatients')
      .then((response) => {
  ...
  function getStatusCountMyPatients() {
    // This will return the number of patients classified under each status for MY patients
  ...
  function getDoctorsWithMostPatients() {
    //This will return the top 5 doctors with most to least patients
    Axios.get('http://localhost:8080/doctorsWithMostPatients')
      .then((response) => {
  ...
  function getDoctorsWithLeastPatients() {
    //This will return the top 5 doctors with least to most patients
    Axios.get('http://localhost:8080/doctorsWithLeastPatients')
      .then((response) => {
  ...
  function getTotalNumberOfDoctors() {
    //This will return the total number of validated doctors
    Axios.get('http://localhost:8080/countAllValidatedDoctors')
      .then((response) => {
  ...
  function getTotalNumberOfPatients() {
    //This will return the total number of validated doctors
    Axios.get('http://localhost:8080/countAllPatients')
      .then((response) => {
  ...
  function getTotalNumberOfFlaggedPatients() {
    //This will return the total number of validated doctors
    Axios.get('http://localhost:8080/countAllFlaggedPatients')
      .then((response) => {
  ...
  function getFlaggedPatientsNotViewed() {
    //This will return the list of patients that have submitted a form but have not been reviewed
    Axios.get('http://localhost:8080/patientsFlaggedNotViewed')
      .then((response) => {
  ...
  function getFlaggedPatientsLeastViewed() {
    //This will return the list of patients whose form has been reviewed from longest to most recent
    Axios.get('http://localhost:8080/patientsFlaggedLeastViewed')
      .then((response) => {
  ...
  function getFlaggedPatientsNoSymptomFormResponse() {
    //This will return the list of patients that have been sent a form to fill out but have not done so
    Axios.get('http://localhost:8080/patientsFlaggedNoSymptomFormResponse')
      .then((response) => {
  ...
  function getAllNotifications() {
    //This will return patient name, and appointment time
  ...
  useEffect(() => {
    //when the doctor dashboard page is rendered, these functions are executed
    getDoctorPatients();
              ...
              {notificationsList.map(
                (val, key) => {
                  return (
                    ...
                    >
                      <CardHeader
                        avatar={<Avatar aria-label=''>{key + 1}</Avatar>}
                  ...
                  <>
                    ...
                    >
                      <CardHeader
                        ...
                        subheader={
                          'New form updated  ' +
                          val.InfoTimestamp.substr(0, 10) +
                          ' at ' +
                          val.InfoTimestamp.substr(11, 8)
                        }
                      />{' '}
                      {/* On click X button, remove notification */}
                      <IconButton
                        aria-label='clear'
                        onClick={handleFormMask(key)}
                      >
                        <ClearIcon />
                      </IconButton>{' '}
                    </CardActions>{' '}
            ...
            <List>
              <Item>
                ...
                {totalDoctorCount.map((val, key) => {
                  return (
                    <ListItem key={key}>
                      <LocalHospitalIcon></LocalHospitalIcon> Total Number of
                      Registered Doctors: {val.allRegisteredDoctorsCount}
                    </ListItem>
                  );
                })}
                <hr></hr>
                ...
                <ListItem>
                  <MedicationIcon></MedicationIcon>Doctors with Most Assigned
                  Patients:
                </ListItem>
                {doctorsWithMostPatientsList.map((val, key) => {
                  return (
                    <ListItem key={key}>
                      {val.Fname} {val.LName} |{' '}
                      <EmailIcon fontSize='small'></EmailIcon> {val.Email} |{' '}
                ...
                <hr></hr>
                <ListItem>
                  <MedicationIcon></MedicationIcon>Doctors with Least Assigned
                  Patients:
                </ListItem>
            ...
            <List>
              <Item>
                ...
                {totalPatientCount.map((val, key) => {
                  return (
                    <ListItem key={key}>
                      <AccessibilityIcon></AccessibilityIcon>Total Number of
                      Registered Patients: {val.allPatientCount}
                    </ListItem>
                  );
                })}
                ...
                {totalFlaggedPatientCount.map((val, key) => {
                  return (
                    <ListItem key={key}>
                      <FlagIcon></FlagIcon>Total Number of Flagged Patients:{' '}
                      {val.allFlaggedPatientCount}
                    </ListItem>
                  );
                })}
                <hr></hr>
                <ListItem>
                  <AccessibilityIcon></AccessibilityIcon>Patients Flagged and
                  Not Viewed:
                </ListItem>
                ...
                <hr></hr>
                <ListItem>
                  <AccessibilityIcon></AccessibilityIcon>Patients Least Recently
                  Viewed:
                </ListItem>
                ...
                <ListItem>
                  <AccessibilityIcon></AccessibilityIcon>Patients Flagged and
                  Require Form Response Immediately:
                </ListItem>
