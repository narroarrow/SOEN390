...
const TimeSlotCalendar = () => {
  ...
  const [previousWednesday, setPreviousWednesday] = useState(
    Array(18).fill(false)
  );
  const [previousThursday, setPreviousThursday] = useState(
    Array(18).fill(false)
  );
  ...
  const handleChange = (event, timeSlot, index) => {
    ...
    switch (timeSlot.day) {
      case 'Monday':
        setPreviousMonday((prevState) =>
          prevState.map((item, idx) => (idx === index ? !item : item))
        );
        break;
      case 'Tuesday':
        setPreviousTuesday((prevState) =>
          prevState.map((item, idx) => (idx === index ? !item : item))
        );
        break;
      case 'Wednesday':
        setPreviousWednesday((prevState) =>
          prevState.map((item, idx) => (idx === index ? !item : item))
        );
        break;
      case 'Thursday':
        setPreviousThursday((prevState) =>
          prevState.map((item, idx) => (idx === index ? !item : item))
        );
        break;
      case 'Friday':
        setPreviousFriday((prevState) =>
          prevState.map((item, idx) => (idx === index ? !item : item))
        );
        break;
    }
    ...
    if (event.target.checked) {
      ...
      previousSlots.splice(
        previousSlots.findIndex(
          (slot) =>
            slot.StartTime.substr(0, 5) === timeSlot.label.substr(0, 5) &&
            slot.dayName === timeSlot.day.substr(0, 3)
        ),
        1
      );
  ...
  const calculateTimeSlots = (startHour, endHour) => {
    const numberOfTimeSlots =
      (endHour - startHour) / (TIME_SLOT_INTERVAL_IN_MINUTES / 60);
    ...
    for (var i = 0; i < DAYS_TO_DISPLAY; i++) {
      ...
      for (var j = 0; j < numberOfTimeSlots; j++) {
        let intervalEnd = moment(incrementedTime).add(
          TIME_SLOT_INTERVAL_IN_MINUTES,
          'm'
        );
        timeSlots.slots.push({
          label: `${incrementedTime.format('hh:mm')} - ${intervalEnd.format(
            'hh:mm'
          )}`,
          day: `${day.format('dddd')}`,
      ...
      .then((response) => {
        ...
        days.forEach((day, index) => {
          ...
          temp.forEach((t) => {
            if (t) {
              dayIndexes.push(
                calculatedTimeSlots[0].slots.findIndex(
                  (s) => s.label.substr(0, 5) === t.StartTime.substr(0, 5)
                )
              );
            }
          });
  ...
  const submit = () => {
      ...
      previousDaysArray[previousDayIndex].forEach((value, index) => {
        if (value === true) {
          backendTimeSlots.push({
            ...
            startTime: timeSlotsPerDay[previousDayIndex].slots[index].label
              .substr(0, 5)
              .concat(':00'),
            endTime: timeSlotsPerDay[previousDayIndex].slots[index].label
              .substr(8, 14)
              .concat(':00'),
          });
        }
    ...
    Axios.post('http://localhost:8080/doctorAvailbility', doctorScheduleData, {
  ...
  return (
    <Box sx={{ p: 10 }}>
      {timeSlotsPerDay.length > 0 &&
        timeSlotsPerDay.map((timeSlotsOnDay, index) => (
          <TimeSlotDayTable
            dayState={
              timeSlotsOnDay.day === 'Monday'
                ? previousMonday
                : timeSlotsOnDay.day === 'Tuesday'
                ? previousTuesday
                : timeSlotsOnDay.day === 'Wednesday'
                ? previousWednesday
                : timeSlotsOnDay.day === 'Thursday'
                ? previousThursday
                : previousFriday
            }
            handleChange={handleChange}
            key={`${index}`}
            day={timeSlotsOnDay.day}
...
const checkChange = (e) => e.target.checked;
